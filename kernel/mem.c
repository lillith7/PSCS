...
static uint8_t *dynamic_mem_area = (uint8_t *)0x01000000; 
// dynamic memory starts past ISA hole for compatbility 
// (may not work on systems without more than 16 MiB memory)
...
void *mem_realloc(void *prev_mem, size_t old_size, size_t new_size) {
    ...
    memcpy(new_ptr, prev_mem, old_size);
    mem_free(prev_mem);
    return new_ptr;
}
